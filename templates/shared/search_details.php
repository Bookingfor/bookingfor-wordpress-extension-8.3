...
if(empty( $resourceId )){
	$resourceId = BFCHelper::getVar('resourceId',0);
}
...
//$url_resource_page =  (!empty($resourcegroupId)) ? BFCHelper::GetPageUrl('resourcegroupdetails') : BFCHelper::GetPageUrl('accommodationdetails');
...
//	}	
				if(!empty($resourcegroupId)) {
				    if ($resourcegroupId == $merchant->DefaultProductGroupId) {
						$currUriresource = $url_merchant_page . $merchant->MerchantId .'-'.BFI()->seoUrl($merchant->MrcName);
					}else{
						
						$currUriresource = $url_groupresource_page . $resourcegroupId.'-'.BFI()->seoUrl($resourceName); 
				    }
				}else{
					switch ($resource->ItemTypeId ) {
						case bfi_ItemType::Experience :
							$currUriresource  = $url_resource_page_experience.$resource->ResourceId.'-'.BFI()->seoUrl($resource->Name);
						break;
						default:      
							$currUriresource = $url_resource_page . $resource->ResourceId .'-'.BFI()->seoUrl($resource->Name) ;
						break;
					} // end switch
...
//	$useSeniores= isset($_REQUEST['seniores']);
	...
	if (!empty($resourceId) && $resource->ItemTypeId== bfi_ItemType::Experience) {
		$totalPerson = count($paxages);
...
$refreshSearch = (isset($refreshSearch) && !$useSeniores) ? $refreshSearch : BFCHelper::getVar('refreshsearch','');
...
if($countPaxes>0){
	foreach ($countPaxes as $key => $count) {
		if ($key >= $defaultAdultsAge) {
			if ($key >= $defaultSenioresAge && $useSeniores) {
				$nse += $count;
			} else {
				$nad += $count;
			}
...
if(!empty($fromSearch) && !empty($makesearch)){
	...
	if (!empty($resultsItems )) {
		...
		if (is_array($tmpAllRatePlans)) {
			foreach ($tmpAllRatePlans as $item) {
			    if (!empty($item->Results)) {
					foreach ($item->Results as $rateplan) {
						if (!empty($rateplan)) {
							
							// assegno i valori mancanti
...
<?php 
if(!empty($variationPlanId) ){
	$formRoute = BFCHelper::getPageUrl('searchavailability');
	$formMethod = "GET";
}

$alternativeformRoute = "";
if(!empty($refreshSearch)){
	$alternativeformRoute = BFCHelper::getPageUrl('searchavailability');
}
...
<?php if($hasPaxes) { ?>
				...
				<?php if($useSeniores && $nse>0) { ?>
					<span id="bfi-senior-info-calculator" class="bfi-comma"><span><?php echo $nse ?></span> <?php echo  sprintf(__('Over %s', 'bfi'), $defaultSenioresAge); ?></span>
				<?php } ?>
				<?php if($nch>0) { ?>
...
<?php } ?>
	...
	<div class="bfi-row bfi_resource-calculatorForm-mandatory ">
			<div class="bfi-row">
					...
					<div class="bfi-row ">
						<div class="bfi-col-md-6 bfi-col-xs-6 bfi-checkin-field-container" id="calcheckin">
							...
							<div class="bfi-datepicker">
								<i class="far fa-calendar-alt"></i> <input name="checkin" type="text" value="<?php echo $checkin->format('d/m/Y'); ?>" id="<?php echo $checkinId; ?>" class="bfi-checkin-field" readonly="readonly" />
							</div>
						...
						<div class="bfi-col-md-6 bfi-col-xs-6 <?php echo ($ProductAvailabilityType == 3 )? "bfi-hide " : " "  ?> bfi-checkout-field-container" id="calcheckout">
							...
							?></label>
							<div class="bfi-datepicker">
								<i class="far fa-calendar-alt"></i> <input type="text" name="checkout" value="<?php echo $checkout->format('d/m/Y'); ?>" id="<?php echo $checkoutId; ?>" class="bfi-checkout-field" readonly="readonly"/>
							</div>
							<?php if($ProductAvailabilityType == 2){  ?>
								<div class="bfi-datetimepicker">
				...
				<div class="bfi-col-md-4 bfi-hidden-xs bfi-hidden-sm bfi-text-center">
...
<?php if($hasPaxes) { ?>									
					...
					<div class="bfi-col-md-2 bfi-col-xs-4 bfi_resource-calculatorForm-senior" >
						<label><?php echo sprintf(__('Over %s', 'bfi'), $defaultSenioresAge);?>:</label><br />
...
<?php } ?>
	...
	<input name="itemtypes" type="hidden" value="<?php echo $itemTypeIds ?>" />
...
<?php } ?>
<?php
if(empty($resourceId) ||( !empty($resourceId) && $resource->ItemTypeId!= bfi_ItemType::Experience) ){
 ?>
...
?>
<?php 
if (!$showResult && empty($resourceId)){
...
?>
		...
		foreach ($resources as $currKey=>$resource){
			foreach ($resource->Results as $resourceId=>$currResource ) {
				...
				$merchantNameTrack =  BFCHelper::string_sanitize($currResource->Name);
				$merchantCategoryNameTrack =  BFCHelper::string_sanitize($merchant->MainCategoryName);
				switch ($currResource->ItemTypeId ) {
					case bfi_ItemType::Experience :
						$currUriresource  = $url_resource_page_experience.$currResource->ResourceId.'-'.BFI()->seoUrl($currResource->Name) . "?lna=".$listNameAnalytics;
					break;
					default:      
						$currUriresource  = $url_resource_page.$currResource->ResourceId.'-'.BFI()->seoUrl($currResource->Name) . "?lna=".$listNameAnalytics;
					break;
				} // end switch
			?>
			<tr>
...
<?php 
				...
				<td><a rel="#divcalculator" class="bfi-btn bfi-alternative bfi-btn-calc" onclick="alert('<?php _e('To see availablility and prices please enter your check-in and check-out dates.', 'bfi') ?>')"><?php echo _e('Show prices','bfi'); ?></a></td>
			</tr>
	<?php 
				}
		}
		...
		<table class="bfi-table bfi-table-bordered bfi-table-resources bfi-table-resources-sticked bfi-table-resourcessearchdetails bfi-table-resources-step1" style="margin-top: 20px;">
			...
			<tr>
				...
				<td rowspan="400" class="bfi-book-now-td">
							<div class="bfi-book-now">
								...
								<div class="bfi-btn bfi-alternative bfi-request-now-first" onclick="bfi_ChangeVariation(this);" style="<?php echo (COM_BOOKINGFORCONNECTOR_ISMOBILE) ?" display:none; ":"";?>">
									<?php _e('Request Now', 'bfi') ?>
								</div>
							</div>
				</td>
			...
			<?php  if(!empty($resourceId) && !in_array($resourceId,$allResourceId)) {
				...
				switch ($resource->ItemTypeId ) {
					...
					break;
					default:      
						$currUriresource  = $url_resource_page.$resource->ResourceId.'-'.BFI()->seoUrl($resourceName) . "?fromsearch=1&lna=".$listNameAnalytics;
					break;
				} // end switch
...
foreach($allResourceId as $resId) {
	...
	if($res->UpSellResult && !isset($upselltitleviewed[$res->UpSellReferrerItemId])) {
		...
		?>
		<tr><td colspan="5">  <?php echo sprintf(__('You may also be interested in the following proposals of "%s"', 'bfi'), ($res->UpSellReferrerItemName)) ?>:</td></tr>
		<?php
	}
...
//	}
				...
				if(!empty($res->ResourceGroupId)) {
				    if ($res->ResourceGroupId == $merchant->DefaultProductGroupId) {
						$currUriresource = $url_merchant_page . $res->MerchantId .'-'.BFI()->seoUrl($res->MrcName)."&fromsearch=1&lna=".$listNameAnalytics . "&checkin=" . $currCheckIn->format("YmdHis") . "&checkout=" . $currCheckOut->format("YmdHis"); ;
					}else{
						
						$currUriresource = $url_groupresource_page . $res->ResourceGroupId .'-'.BFI()->seoUrl($res->ResourceGroupName). "?fromsearch=1&lna=".$listNameAnalytics; 
				    }
				}else{
					switch ($res->ItemTypeId ) {
						...
						break;
						default:      
							$currUriresource = $url_resource_page . $res->ResourceId .'-'.BFI()->seoUrl($res->Name) . "?fromsearch=1&lna=".$listNameAnalytics;
						break;
					} // end switch
...
//										$currCheckIn = DateTime::createFromFormat('Ymd', $currDatesTimeSlot[0]->StartDate,new DateTimeZone('UTC'));
										...
										//nuovo sistema
										$currDatesTimeSlot = $res->AvailableTimes;

//echo "<pre>";
//echo print_r($currDatesTimeSlot[0]);
//echo "</pre>";
//
										$currCheckIn = DateTime::createFromFormat('Y-m-d\TH:i:s', $currDatesTimeSlot[0]->StartDate,new DateTimeZone('UTC'));
...
//											$strDuration .= sprintf(__(' %d day/s' ,'bfi'), $currDiff->d);
									...
									?>
										<div class="bfi-timeslot bfi-cursor" id="bfi-timeslot-<?php echo $res->ResourceId ?>" 
											...
											data-timeslotid="<?php echo $currDatesTimeSlot[0]->TimeSlotId ?>" 
...
//										return $itm->StartDate == $currCheckIn->format("Ymd");
										return $itm->StartDate == $currCheckIn->format("Y-m-d\T00:00:00");
									}));
										...
										foreach ($timeSlotList as $singleSlot ) {
											...
											?>
															<li><a href="javascript:void(0);" onclick="updateTotalTimeSlotList(this)" 
																data-resid="<?php echo $res->ResourceId ?>" 
																data-ProductId="<?php echo $singleSlot->TimeSlotId ?>"  
...
//Calcolo
	...
	foreach($resRateplans as $rpKey => $currRateplan) {
		...
		$currExcludedPriceTypeValues = isset($currRateplan->RatePlan->ExcludedPriceTypeValues)?(array)$currRateplan->RatePlan->ExcludedPriceTypeValues:[];
		...
		if(!empty($currRateplan->RatePlan->AllVariationsString)){
			...
			// ciclo se c'è una offerta aggiorno la promo per selectablePrices
			foreach ($allVar as $promotion ) {
		...
		if ($policy->CanBeCanceled) {
			...
			if (!empty($policy->CanBeCanceledCurrentTime )){
				if (!empty($policy->CancellationTime)) {
					...
					$currDatePolicy = BFCHelper::GetDateTimeFromDurationDiff($currCheckIn, $policy->CancellationTime);
					$currTimeBefore = BFCHelper::GetDurationString($policy->CancellationTime);
				}
			...
			<tr id="data-id-<?php echo $currRateplan->ResourceId ?>-<?php echo $currRateplan->RatePlan->RatePlanId ?>-<?php echo $resRef ?>" class="<?php echo $IsBookable?"bfi-bookable":"bfi-canberequested"; ?>" style="display:<?php echo (($currRow >= $hideRateplanOver)?'none':'') ?>">
				...
				<td><!-- price -->
					...
					<?php if(!COM_BOOKINGFORCONNECTOR_ISMOBILE) { ?>
							...
							<div data-value="<?php echo $currRateplan->Price ?>" class="bfi-price  <?php echo ($currRateplan->Price < $currRateplan->TotalPrice ? "bfi-red" : "") ?> bfi_<?php echo $currencyclass ?>"><?php echo BFCHelper::priceFormat($currRateplan->Price) ?> 
							<?php 
...
?>
					...
					data-maxqt="<?php echo min($currRateplan->MaxQt, COM_BOOKINGFORCONNECTOR_MAXQTSELECTABLE)?>"
					>
					<?php 
...
<table class="bfi-table bfi-table-bordered bfi-table-resources bfi-table-selectableprice bfi-table-selectableprice-container bfi-table-resources-sticked" style="margin-top: 20px;">
			<thead>
				<tr>
					<th><div><?php _e('Do you want add more?', 'bfi') ?></div></th>
...
//		$currUriresource = $uri.$currRateplan->ResourceId . '-' . BFCHelper::getSlug($currRateplan->ResName) . "?fromsearch=1&lna=".$listNameAnalytics . "&checkin=" . $currCheckIn->format("YmdHis") . "&checkout=" . $currCheckOut->format("YmdHis");
		switch ($currRateplan->ItemTypeId ) {
			...
			break;
			default:      
				$currUriresource  = $url_resource_page.$currRateplan->ResourceId.'-'.BFI()->seoUrl($currRateplan->ResName) . "?fromsearch=1&lna=".$listNameAnalytics . "&checkin=" . $currCheckIn->format("YmdHis") . "&checkout=" . $currCheckOut->format("YmdHis");
			break;
		} // end switch
						...
						<?php if (isset($selPrice->CalculationType) && !empty($selPrice->CalculationType)){?>
							<?php 
								if (($nad + $nse)>0) {
									?>
									<div class="bfi-icon-paxes">
										<i class="fa fa-user"></i> x <b><?php echo ($nad + $nse)  ?></b>
									<?php 
										if ($nch>0) {
											?>
											+ <br />
												<span class="bfi-redux"><i class="fa fa-user"></i></span> x <b><?php echo $nch ?></b>
											<?php 
...
//		}
		jQuery('.bfi-options-help i').webuiPopover({trigger:'hover',placement:'auto',style:'bfi-webuipopover'});
...
?>
	if (whatsearch !== undefined &&  whatsearch == 'refreshsearch' && <?php echo (COM_BOOKINGFORCONNECTOR_ENABLEGENERICSEARCHDETAILS) ? "true":'false' ?> )
	{
		var currDateFormat = "dd/mm/yy";
		currCheckin.datepicker( "option", "dateFormat",currDateFormat );
		currCheckout.datepicker( "option", "dateFormat", currDateFormat );
...
function bfiUpdateInfoResGrp(){
	jQuery(".bfiresourcetags, .bfiresourcetagshighlighted").each(function(){
		...
		if (currList!= null && currList!= '' || currArea>0)
		{
			...
			if (currArea>0)
			{
				srvArr.push('<span class="bfi-border-area"><i class="far fa-expand-arrows-alt"></i></span> '+ currArea + ' m²');
			}
