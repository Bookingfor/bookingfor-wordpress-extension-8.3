...
$onlystay =  true;
$currParam = BFCHelper::getSearchParamsSession('search.params.rental');
...
//		$listResourceMaps[] = $val;
    ...
    ?>
	<div class="bfi-col-sm-6 bfi-item bfi-list-group-item" data-href="<?php echo $itemRoute ?>" style="display:<?php echo ($resource->Available) ?"":"none"; ?>">
...
?>
						...
						<?php if($resource->ItemType!=1 && Count($resource->Results)>0) { ?>
							<?php
							$currResource = $resource->Results[0];
							if (isset($currResource->MaxPaxes) && !empty($currResource->MaxPaxes)) {?>
								<div class="bfi-icon-paxes">
									<i class="fa fa-user"></i> <?php echo  $currResource->MaxPaxes ?>
								</div>
							<?php
								}
							?>							
...
?>
				<div>
						<div class="bfi-result-singleitem bfi-row">
							...
							<a href="<?php echo $resourceRoute?>" onclick="event.stopPropagation();" class="bfi-subitem-title eectrack" <?php echo COM_BOOKINGFORCONNECTOR_TARGETURL ?> data-type="Resource" data-id="<?php echo $currResource->ResourceId?>" data-index="<?php echo $counter?>" data-itemname="<?php echo $currresourceNameTrack; ?>" data-category="<?php echo $merchantCategoryNameTrack; ?>" data-brand="<?php echo $merchantNameTrack; ?>"><b><?php echo (count($resource->Results) > 1 ? count($singleResource) . " x " : "") .$currName; ?></b> <span class="bfi-rental-caption"><?php _e('or similar', 'bfi') ?></span></a> 
							<div class="bfi-result-singleitem-details">								
							</div>
							<div class="bfi-result-singleitem-termofuse" id="bfitermofuse<?php echo $currResource->ResourceId?>" style="display:none">
								<div class="bfi-result-singleitem-termofuse-title" title="<?php echo  _e('Important information', 'bfi') ?>"><?php _e('Rental terms', 'bfi') ?></div>
								<div class="bfi-result-singleitem-termofuse-content " id="bfitermofusecontent<?php echo $currResource->ResourceId?>" style="display:none">
								</div>
							</div>
...
//					    echo "Multi risorsa -> form<br />";
					...
					?>
					<form method="post" action="<?php echo $itemRoute ?>" <?php echo COM_BOOKINGFORCONNECTOR_TARGETURL ?> >
