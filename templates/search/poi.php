...
if (!empty($currParam)){
		...
		$poiName = isset($currParam['poiName']) ? $currParam['poiName'] : '' ;
...
if(!empty( $newsearch )){
	...
	$track = array($total,$tagids,$merchantIds,$stateIds,$regionIds,$cityIds);
...
if (typeof(ga) !== 'undefined') {
	ga('send', 'poi', 'Bookingfor - SearchPoi', 'Search', '<?php echo $trackstr ?>');
}
...
<?php if ($total > 0){ ?>
	<?php 
			...
			bfi_get_template("search/poi_results.php",array("results"=>$results,"total"=>$total,"items"=>$items,"pages"=>$pages,"page"=>$page,"listNameAnalytics"=>$listNameAnalytics,"currSorting"=>$currSorting));	
...
//			$link =  add_query_arg('searchid' , $searchid , $link);
