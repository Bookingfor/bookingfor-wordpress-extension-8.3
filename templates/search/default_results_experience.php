...
//$fromsearchparam = "/?fromsearch=0&lna=".$listNameAnalytics;
...
$onlystay =  true;
$currParam = BFCHelper::getSearchParamsSession('search.params.experience');
...
$url_resource_page = BFCHelper::GetPageUrl('experiencedetails');
...
//					echo " ". __('on', 'bfi') . " " . $total ." ";
	...
	<div class="bfi-search-menu">
		...
		<div class="bfi-results-sort">
			...
			<span class="bfi-sort-item <?php echo $currSorting=="rating|desc" ? "bfi-sort-item-active": "" ; ?>" rel="rating|desc" ><?php echo _e('Supplier rating' , 'bfi'); ?></span>
...
//		$listResourceMaps[] = $val;
	...
	$setFavDate = 0;
	if($resource->Available && $resource->TotalPrice>0 && Count($resource->Results)>0 && !empty($currParam['checkAvailability'])) { //ok disp 
...
?>
						...
						<div class="bfi-item-address bfi-hide">
...
?>
						...
						<div class="bfi-description bfi-shortentextlong" id="bfiresdescription<?php echo $resource->ItemId; ?>"></div>
						<div class="bfi-experiencetimelength" id="bfiexperiencetimelength<?php echo $resource->ItemId; ?>"></div>
						<div class="bfi-liveguidelanguages" id="bfiliveguidelanguages<?php echo $resource->ItemId; ?>"></div>
					</div>
...
<?php 
	...
	foreach ($listResources as $resourceId=>$singleResource) // foreach $listMerchantsCart
	{
		...
		if ($currResource->Availability > 0 && $currResource->TotalPrice > 0 && !empty($currParam['checkAvailability']) &&  isset( $currResource->AvailabilityDate) &&  isset( $currResource->CheckOutDate) ) {
			$currCheckIn = DateTime::createFromFormat('Y-m-d\TH:i:s',$currResource->AvailabilityDate,new DateTimeZone('UTC'));
			$currCheckOut = DateTime::createFromFormat('Y-m-d\TH:i:s',$currResource->CheckOutDate,new DateTimeZone('UTC'));
			if (isset( $currCheckIn) && isset( $currCheckOut)) {
...
?>
				<div class="bfi-result-singleitem bfi-row">
					<div class="bfi-col-sm-8">
						<a href="<?php echo $resourceRoute?>" onclick="event.stopPropagation();" class="bfi-hide bfi-subitem-title eectrack" <?php echo COM_BOOKINGFORCONNECTOR_TARGETURL ?> data-type="Resource" data-id="<?php echo $currResource->ResourceId?>" data-index="<?php echo $counter?>" data-itemname="<?php echo $currresourceNameTrack; ?>" data-category="<?php echo $merchantCategoryNameTrack; ?>" data-brand="<?php echo $merchantNameTrack; ?>"><b><?php echo (count($resource->Results) > 1 ? count($singleResource) . " x " : "") .$currName; ?></b> <span class="bfi-rental-caption"></span></a> 
						<div class="bfi-result-singleitem-details">	
						<?php 
							if (!empty($resource->AvailableTimes) && count($resource->AvailableTimes)>0) {
								$timeSlotList = array_values(array_filter($resource->AvailableTimes, function($itm) use ($currCheckIn) {
									return $itm->StartDate == $currCheckIn->format("Y-m-d\TH:i:s");
								}));
								if (!empty($timeSlotList) && count($timeSlotList)>0) {
									?>
										<div class="bfi-slot-list-title"><i class="fa fa-calendar"></i> <?php echo $currCheckIn->format("d") ?> <?php echo date_i18n('M',$currCheckIn->getTimestamp()) ?> </div>
									...
									foreach ($timeSlotList as $singleSlot ) {
										...
										?>
														<li><a href="<?php echo $itemRoute.$searchparamsuffix ?>&timeSlotId=<?php echo $singleSlot->TimeSlotId  ?>" onclick="event.stopPropagation();" <?php echo COM_BOOKINGFORCONNECTOR_TARGETURL ?> class="eectrack" data-type="<?php echo $resourceDataTypeTrack ?>" data-id="<?php echo $resourceDataIdTrack?>" data-index="<?php echo $currKey?>" data-itemname="<?php echo $resourceNameTrack; ?>" data-category="<?php echo $merchantCategoryNameTrack; ?>" data-brand="<?php echo $merchantNameTrack; ?>" >
						...
						<?php if (!$currResource->IsCatalog && $onlystay && !empty($currParam['checkAvailability'])&& !empty($currParam['dateselected'])  ){ ?>
					...
					<div class="bfi-col-sm-4  bfi-text-right  bfi-containerPrice">
						<?php 
						if ($resource->Price>0 && !$currResource->IsCatalog && $onlystay && !empty($currParam['checkAvailability'])&& !empty($currParam['dateselected'])) {
							$val->Price = $resource->Price;
							$val->TotalPrice = $resource->TotalPrice;
							$val->Currencyclass = $currencyclass;

							if (count($currResource->RatePlan->AllVariations)>0) {
