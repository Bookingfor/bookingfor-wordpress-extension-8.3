...
$onlystay =  true;
$currParam = BFCHelper::getSearchParamsSession('search.params.slot');
							...
							<?php 
								if (!empty($resource->AvailableTimes) && count($resource->AvailableTimes)>0) {
									...
									if (!empty($timeSlotList) && count($timeSlotList)>0) {
										?>
											<div class="bfi-slot-list-title"><?php _e('Time Table', 'bfi')  ?>: <?php echo $currCheckIn->format("d/m/Y"); ?> </div>
