...
$searchAvailability_page = get_post( bfi_get_page_id( 'searchpoi' ) );
if(!empty($searchAvailability_page)){
...
$accommodationdetails_page = get_post( bfi_get_page_id( 'pointsofinterestdetails' ) );
...
foreach ($resources as $currKey => $res){
	...
	$resourceDataTypeTrack = "Poi";

	$showResourceMap = (!empty($res->Address->XPos) && !empty($res->Address->YPos)) && !empty(COM_BOOKINGFORCONNECTOR_ENABLEGOOGLEMAPSAPI);
	$val= new StdClass;
	if ($showResourceMap) {
		$val->Id = $res->PointOfInterestId;
		$val->Lat = $res->Address->XPos;
		$val->Long = $res->Address->YPos;
		$val->infoHtml = "<div class='bfi-map-label bfi-googlemap blulighttheme'><div class='bfi-map-label-content bfi-map-poi'>" . $res->Name ."</div><div class='bfi-map-label-arrow'></div></div>";
	}
	$itemRoute = "";
	$imageUrl = COM_BOOKINGFORCONNECTOR_DEFAULTIMAGE;

	$itemRoute = $url_resource_page.$res->PointOfInterestId.'-'.BFI()->seoUrl($resourceName).$fromsearchparam;

	if(!empty($res->DefaultImg)){
		$imageUrl = BFCHelper::getImageUrlResized('poi',$res->DefaultImg, 'medium');
	}

	$resourceRoute = $itemRoute;

	if (isset($res->PointOfInterestType) && !empty($res->ReferrerObjectId) && !empty($res->ReferrerObjectName) ) {
		switch ( $res->PointOfInterestType ) {
			case 0 : //Merchant
				$itemRoute = $url_merchant_page . $res->ReferrerObjectId  .'-'.BFI()->seoUrl($res->ReferrerObjectName ).$fromsearchparam;
				break;
			case 1 : //Product 
				$itemRoute = $url_accommodationdetails_page.$res->ReferrerObjectId .'-'.BFI()->seoUrl($res->ReferrerObjectName ).$fromsearchparam;
				break;
			case 2 : //Event  
				$itemRoute = $url_eventdetails_page.$res->ReferrerObjectId .'-'.BFI()->seoUrl($res->ReferrerObjectName ).$fromsearchparam;
				break;
			case 3 : //EventDate   
				$itemRoute = $url_eventdetails_page.$res->ReferrerObjectId .'-'.BFI()->seoUrl($res->ReferrerObjectName ).$fromsearchparam;
				break;
...
?>
			...
			<div class="bfi-col-sm-9 bfi-details-container">
				...
				<div class="bfi-row" >
					<div class="bfi-col-sm-12">
						<div class="bfi-item-title">
							<a href="<?php echo $itemRoute ?>" onclick="event.stopPropagation();"  id="nameAnchor<?php echo $res->PointOfInterestId?>" <?php echo COM_BOOKINGFORCONNECTOR_TARGETURL ?> class="eectrack" data-type="<?php echo $resourceDataTypeTrack ?>" data-id="<?php echo $res->PointOfInterestId?>" data-index="<?php echo $currKey?>" data-itemname="<?php echo $resourceNameTrack; ?>" data-category="<?php echo $merchantCategoryNameTrack; ?>" data-brand="<?php echo $merchantNameTrack; ?>"><?php echo  $resourceName ?></a> 
						</div>
						<div class="bfi-item-address">
							<?php if ($showResourceMap){?>
							<i class="fa fa-map-marker fa-1"></i> <a href="javascript:void(0);" onclick="event.stopPropagation();bookingfor.bfiShowMarker(<?php echo $res->PointOfInterestId?>)"><span id="address<?php echo $res->PointOfInterestId?>"></span>
							...
							<div class="bfi-hide" id="markerInfo<?php echo $res->PointOfInterestId?>">
									...
									<div style="padding:5px;">
										...
										<span id="mapaddress<?php echo $res->PointOfInterestId?>"></span>
										<div class="bfi-text-right"><a href="<?php echo $itemRoute ?>" onclick="event.stopPropagation();"  <?php echo COM_BOOKINGFORCONNECTOR_TARGETURL ?> class="bfi-btn eectrack" data-type="<?php echo $resourceDataTypeTrack ?>" data-id="<?php echo $res->PointOfInterestId?>" data-index="<?php echo $currKey?>" data-itemname="<?php echo $resourceNameTrack; ?>" data-category="<?php echo $merchantCategoryNameTrack; ?>" data-brand="<?php echo $merchantNameTrack; ?>"><?php _e('View', 'bfi') ?></a> </div>
									</div>
							</div>
							<?php if(isset($res->CenterDistance)) { ?>
								<span class="bfi-centerdistance" id="addressdist<?php echo $res->PointOfInterestId?>" data-toggle="tooltip" title="<?php _e('This is the straight-line distance on the map. Actual travel distance may vary.', 'bfi') ?>">(<i class="fa fa-road" aria-hidden="true"></i> <?php echo BFCHelper::formatDistanceUnits($res->CenterDistance)?> <?php _e('from centre', 'bfi') ?>)</span>
							<?php } ?>

							<?php } ?>
						</div>
                        <div class="bfi-item-descr bfi-description bfi-shortentextlong" id="descrInfo<?php echo $res->PointOfInterestId?>" >
                ...
                <div class="bfi-clearfix bfi-hr-separ"></div>
					...
					?>
					<div class="bfirestags" id="bfitags<?php echo $res->PointOfInterestId?>" rel="<?php echo $res->TagsIdList ?>"></div>
