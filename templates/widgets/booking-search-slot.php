...
if ( ! defined( 'ABSPATH' ) ) {
	...
	$currModID = uniqid('bfisearchslot');
...
if(BFI()->isSearchPage()){
	bfi_setSessionFromSubmittedData('search.params.slot');
}
$parsResource = BFCHelper::getSearchParamsSession('search.params.slot');
...
$defaultdurationSelected =  ( ! empty( $instance['defaultduration'] ) ) ? $instance['defaultduration'] : 1;
//$checkoutspan = '+1 day';
...
$currModID = 'slot'. $currId;
$totalfields=0;

?>
	<div class="bfi-mod-bookingforsearch-slot <?php echo ( ! empty( $fixedonbottom ) ) ? 'bfiAffixBottom' : '' ?> <?php echo $customclass ?>" id="bfisearchslot<?php echo $currModID ?>" >
			<form action="<?php echo $url_page_Resources ?>" method="get"" method="get" id="searchformslot<?php echo $currModID ?>" class="bfi-form-slot bfi-form-default bfi-dateform-container bfi-form-<?php echo $showdirection?"horizontal":"vertical"; ?> " 
			...
			>
				...
				?>
					<div class="bfi_destination bfi-col-sm-2">
						...
						<input type="text" id="searchtext<?php echo $currModID ?>" name="searchterm" value="<?php echo $searchterm ?>" class="bfi-inputtext bfi-autocomplete" placeholder="<?php _e('Search text', 'bfi') ?>" data-scope="<?php echo $searchTextFields ?>" inputmode="search" />
					</div>
				<?php }//$showSearchText ?>
				...
				?>
					...
					<div class="bfi-showdaterange bfi-checkout-field-container bfi-col-sm-2" id="divcheckoutsearch<?php echo $currModID ?>"style="display:none">
...
?>
				...
				<input type="hidden" name="availabilitytype" value="3" />
				...
				<input type="hidden" name="resview" value="slot" />
			...
			<div class="bficontainerviewform">
				<a class="bfiviewform" rel="bfi-form-slot"><i class="fa fa-search"></i> <?php _e('Search', 'bfi') ?> </a>
			</div>
</div>
<?php 
