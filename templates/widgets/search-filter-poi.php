...
if(BFI()->isSearchPoisPage() || !empty($showfilter)) {
    ...
    $currSorting = "";
    $pars = BFCHelper::getSearchPoiParamsSession();
    ...
    if (empty($GLOBALS['bfPoiSearched'])) {
        if($newsearch == "1"){
    ...
    $filtersSelected = BFCHelper::getFilterSearchPoiParamsSession();
    $firstFilters = BFCHelper::getFirstFilterSearchPoiParamsSession();
    $currentFilters = BFCHelper::getEnabledFilterSearchPoiParamsSession();
...
<div class="bfi-searchfilter">
    ...
    <form action="<?php echo $formAction; ?>" method="get" id="searchPoiformfilter" name="searchPoiformfilter">
        <input type="hidden" value="<?php echo $searchid ?>" name="searchid" />
        <input type="hidden" value="<?php echo uniqid('', true)?>" name="Pois" />
            ...
            <div class="bfi-filter-group <?php echo !$showFilters?"bfi-hide":"";?>" rel="<?php echo $filter->PropertyName?>">
              ...
              foreach ($filter->Items  as $currItem){
                  ...
                  $viewCount = $viewResidual?($currItem->ResidualCount >0 ? "+" . $currItem->ResidualCount : ""): ($currItem->Count >0 ? $currItem->Count : "")
                    ?>
                    <a href="javascript:void(0);" rel="<?php echo $currItem->Id ?>" rel1="<?php echo  $propName?>" data-list="Poi Filters" class="<?php echo $currItem->Selected ?"bfi-filter-active":"";?> ">
...
function bfi_applyfilterPoidata(){

	jQuery('#searchPoiformfilter').submit(function( Poi ) {
				try
				{
		...
		jQuery("#searchPoiformfilter").find("input[rel^='filters\\[']").val("");
		jQuery.each(Object.keys(allFilters), function (i, itm) {
			jQuery("#searchPoiformfilter").find("input[rel='filters\\[" + itm + "\\]']").val(allFilters[itm].map(function (gr) { return gr.value; }).join(','));
		});
		jQuery('body').block({
			message: "",
