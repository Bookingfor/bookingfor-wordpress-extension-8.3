...
//		$unitCategoryId = $params['unitCategoryId'];
		$unitCategoryId = isset($params['unitCategoryId']) ? $params['unitCategoryId'] : '';
		$merchantId = isset($params['merchantId']) ? $params['merchantId'] : '';

//		$contractTypeId = $params['contractTypeId'];
		$contractTypeId = isset($params['contractTypeId']) ? $params['contractTypeId'] : '';
...
//		$zoneId = $params['zoneId'];
		$zoneId = isset($params['zoneId']) ? $params['zoneId'] : '' ;
...
//		$searchid = $params['searchid'];
		$searchid = isset($params['searchid']) ? $params['searchid'] : uniqid('', true) ;
...
//			$this->setState('params',$params );	
		...
		BFCHelper::setSearchOnSellParamsSession($this->params );
			...
			$this->params  = array(
				'searchid' => $searchid,
				'contractTypeId' => (!empty($pars['contractTypeId']))?$pars['contractTypeId']:"",
				'unitCategoryId' => (!empty($pars['unitCategoryId']))?$pars['unitCategoryId']:"",
				'merchantId' => (!empty($pars['merchantId']))?$pars['merchantId']:0,
				'zoneId' => BFCHelper::getVar('zoneId',0),
				'cityId' => (!empty($pars['cityId']))?$pars['cityId']:0,
				'zoneIds' => (!empty($pars['zoneIds']))?$pars['zoneIds']:0,
//				'locationzone' =>  BFCHelper::getVar('zoneId'),
				'areamin' => (!empty($pars['areamin']))?$pars['areamin']:"", 
				'areamax' => (!empty($pars['areamax']))?$pars['areamax']:"",
				'cultureCode' => (!empty($pars['cultureCode']))?$pars['cultureCode']:"", 
				'pricemin' => (!empty($pars['pricemin']))?$pars['pricemin']:null,
				'pricemax' => (!empty($pars['pricemax']))?$pars['pricemax']:null,
				'roomsmin' => (!empty($pars['roomsmin']))?$pars['roomsmin']:null,
				'roomsmax' => (!empty($pars['roomsmax']))?$pars['roomsmax']:null,
				'bedroomsmin' => (!empty($pars['bedroomsmin']))?$pars['bedroomsmin']:null, 
				'bedroomsmax' => (!empty($pars['bedroomsmax']))?$pars['bedroomsmax']:null,
				'bathsmin' => (!empty($pars['bathsmin']))?$pars['bathsmin']:null,
				'bathsmax' => (!empty($pars['bathsmax']))?$pars['bathsmax']:null,
				'searchType' => (!empty($pars['searchType']))?$pars['searchType']:"",
				'points' => (!empty($pars['points']))?$pars['points']:null,
				'services' => (!empty($pars['services']))?$pars['services']:null,
				'isnewbuilding' => (!empty($pars['isnewbuilding']))?$pars['isnewbuilding']:"",
				'randomresult' => (!empty($pars['randomresult']))?$pars['randomresult']:"",
				'showReserved' => (!empty($pars['showReserved']))?$pars['showReserved']:"", 
				'searchseed' => (!empty($pars['searchseed']))?$pars['searchseed']:"",
