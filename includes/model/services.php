...
//			$services = $res->d->results ?: $res->d;
	...
	public function getServicesByIds($listsId,$language='') {// with randor order is not possible to otrder by another field
		...
		$services = $this->getServicesFromService($language);
		if(!empty( $services ) && !empty( $listsId ) ){
			$aListsId = explode (",",$listsId);

			$services = array_filter($services, function($singleService) use($aListsId) {
					if (in_array($singleService->ServiceId,$aListsId)) {
