...
class BookingForConnectorModelResources
{
	...
	private $urlGetCrossSellResourcesByIds = null;
	...
	public function __construct($config = array())
	{
      $ws_url = COM_BOOKINGFORCONNECTOR_WSURL;
		...
		$this->urlGetCrossSellResourcesByIds = '/GetCrossSellResourcesByIds';
	}
	...
	public function applyDefaultFilter(&$options) {
		...
		$resourcegroupid = $params['parentProductId'];
		if (!empty($resourcegroupid)) {
			$options['data']['parentProductId'] =  $resourcegroupid;
		}
...
//		if (!$session->has('getMerchantCategories','com_bookingforconnector')) {
	...
	public  function GetCrossSellResourcesByIds($listsId,$language='',$start, $limit) {
		$options = array(
				'path' => $this->urlGetCrossSellResourcesByIds,
				'data' => array(
					'ids' => '\'' .$listsId. '\'',
					'cultureCode' => BFCHelper::getQuotedString($language),
					'$format' => 'json'
		...
		if (isset($r)) {
			...
			if (!empty($resources)) {
				foreach($resources as $resource) {
					if (!empty($resource->Resource->TermsOfUse)) {
						$resource->Resource->TermsOfUse = BFCHelper::getLanguage($resource->Resource->TermsOfUse, $language, null, array('striptags'=>'striptags', 'bbcode'=>'bbcode','ln2br'=>'ln2br'));
					}
					if (!empty($resource->Resource->LiveGuideLanguages)) {
						$currLanguages = array();
						foreach(explode(",",$resource->Resource->LiveGuideLanguages) as $currLang) {
							$currLanguages[] = locale_get_display_language($currLang,$GLOBALS['bfi_lang']);
						}			
						$resource->Resource->LiveGuideLanguages = implode(", ",$currLanguages). '</div>';
					}
					if(isset($resource->Resource->ExperienceTimeLength) && $resource->Resource->ExperienceTimeLength>0  ){ 
						$experienceTime = $resource->Resource->ExperienceTimeLength/(60*24);
						$experienceTimeLabel =  __('Days', 'bfi');
						if ($experienceTime<1) {
							$experienceTime = $resource->Resource->ExperienceTimeLength/60;
							$experienceTimeLabel =  __('Hours', 'bfi') . " " . __('(Approx.)', 'bfi');
						}
						$experienceTime = round($experienceTime, 0, PHP_ROUND_HALF_UP);
						$resource->Resource->ExperienceTime = $experienceTime . ' ' . $experienceTimeLabel;
					}
