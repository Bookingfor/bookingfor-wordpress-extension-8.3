...
class BookingForConnectorModelResource
{
	...
	private $urlResourcePolicies = null;
	...
	public function __construct($config = array())
	{
      $ws_url = COM_BOOKINGFORCONNECTOR_WSURL;
		...
		$this->urlResourcePolicies = '/GetPolicyByResourceId';// 
...
//		}
		...
		if(!empty($resource)){
			...
			$resource->Policies = $this->GetPolicyByResourceId($resource_id);
//			$resource->Policies= "";
	...
	public function getTagsByResourceId($resourceId,$viewContextType=null){
...
//		$cultureCode =  'en-gb';
	...
	*/
	...
	public function GetPolicyByResourceId($resourceId, $cultureCode=null) {
		if(empty($cultureCode)){
			$cultureCode = $GLOBALS['bfi_lang'];
		}
		$options = array(
			'path' => $this->urlResourcePolicies,
			'data' => array(
				'resourceId' => $resourceId,
				'cultureCode' => '\'' . $cultureCode . '\'',
				'$format' => 'json'
...
//		$r = $this->helper->executeQuery($url,null,null,false);
		...
		if (isset($r)) {
			$res = json_decode($r);
			if (!empty($res->d->GetPolicyByResourceId)){
				$types = $res->d->GetPolicyByResourceId;
			}elseif(!empty($res->d)){
				$types = $res->d;
			}
		}
...
//				'path' =>  $this->urlGetRelatedResourceStays,
			...
			if(isset( $relatedProductid ) && !empty($relatedProductid)){
				$orderby = 'resourceid:' . $relatedProductid . ":1|price";
				$options['data']['orderby'] = '\'' . $orderby . '\'';
			}else{
				$options['data']['orderby'] = '\'' . $orderby . '|price\'';
...
//	public function getSearchResults($start, $limit, $ordering, $direction, $merchantid, $resourcegroupid = null, $ignorePagination = false, $jsonResult = false, $excludedResources = array(), $requiredOffers = array(), $overrideFilters = null,$language='') {
//
