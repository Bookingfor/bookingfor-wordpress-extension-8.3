...
class BookingForConnectorModelOrders
{
	...
	private $urlUpdateOrderCreditCardData = null;
        ...
        private $urlGetCartByExternalUserSimple = null;
        ...
        private $urlAddToCartSimple = null;
        private $urlUpdateCartExternalUser = null;
	...
	public function __construct($config = array())
	{
		...
		$this->urlGetCartByExternalUserSimple = '/GetCartByExternalUserSimple';
		...
		$this->urlAddToCartSimple = '/AddToCartSimple';
		$this->urlUpdateCartExternalUser = '/UpdateCartExternalUser';
...
//		}
	...
	public function setOrder($customerData = NULL, $suggestedStay = NULL, $creditCardData = NULL, $otherNoteData = NULL, $merchantId = 0, $orderType = NULL, $userNotes = NULL, $label = NULL, $cultureCode = NULL, $processOrder = NULL, $priceType = NULL, $merchantBookingTypeId = NULL, $policyId = NULL, $crewData = NULL) {
...
//		}
		...
		$options = array(
				...
				'data' => array(
					'customerData' => BFCHelper::getQuotedString(BFCHelper::getJsonEncodeString($customerData)),
					'crewData' => BFCHelper::getQuotedString(BFCHelper::getJsonEncodeString($crewData)),
...
//		}
	...
	public function GetCartByExternalUserSimple($userId, $culturecode, $includeDetails = true) {
//		$uid = get_current_user_id();
//		$user = get_user_by('id', $uid);
////		if (empty($user->ID) || empty($userId) ) return null;
//
...
//		}
		...
		$options = array(
			'path' => $this->urlGetCartByExternalUserSimple,
			'data' => $data
		);
		$url = $this->helper->getQuery($options);
		...
		if (isset($r)) {
			$res = json_decode($r);
			if (!empty($res->d->GetCartByExternalUserSimple)){
				$order = $res->d->GetCartByExternalUserSimple;
			}elseif(!empty($res->d)){
				$order = $res->d;
			}
			if(!empty($order)){
	...
	public function UpdateCartExternalUser($oldUserId) {
		$userId = BFCHelper::bfi_get_userId();
		$data = array(
		...
		$options = array(
			'path' => $this->urlUpdateCartExternalUser,
			'data' => $data
		);
		$url = $this->helper->getQuery($options);
		...
		if (isset($r)) {
			$res = json_decode($r);
			if (!empty($res->d->UpdateCartExternalUser)){
				$result = $res->d->UpdateCartExternalUser;
			}elseif(!empty($res->d)){
				$result = $res->d;
			}
		}
	...
	public function AddToCartSimple($userId, $culturecode, $cartDetails, $resetCart = 0) {
		$data = array(
			'UserId' => BFCHelper::getQuotedString($userId),
			'domainLabel' => BFCHelper::getQuotedString(COM_BOOKINGFORCONNECTOR_FORM_KEY),
			'cultureCode' => BFCHelper::getQuotedString($culturecode),
		...
		if (isset($r)) {
			$res = json_decode($r);
			if (!empty($res->d->AddToCartSimple)){
				$cart = $res->d->AddToCartSimple;
			}elseif(!empty($res->d)){
				$cart = $res->d;
			}
			if (!empty($cart) && !empty($cart->CartTotalItems)) {
				BFCHelper::setSession('totalItems', $cart->CartTotalItems, 'bfi-cart');
			}
		}
