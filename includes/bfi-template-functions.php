...
//add_action( 'template_redirect', 'bfi_prevent_endpoint_indexing' );
		...
		if (BFI()->isMerchantPage() ||  BFI()->isResourcegroupPage() || BFI()->isResourcePage() || BFI()->isResourceOnSellPage() || BFI()->isCartPage() || BFI()->isEventPage()  ||  BFI()->isExperiencePage()) {
			if(!empty(COM_BOOKINGFORCONNECTOR_GOOGLE_GOOGLERECAPTCHAKEY)){
				switch (COM_BOOKINGFORCONNECTOR_GOOGLE_GOOGLERECAPTCHAVERSION) {
					case "V3":
						 ?> <script src="https://www.google.com/recaptcha/api.js?render=<?php echo COM_BOOKINGFORCONNECTOR_GOOGLE_GOOGLERECAPTCHAKEY ?>"></script>
						 <?php 					 
						break;
					default:
	...
	function bfi_google_analytics_EEc($listName ) {
		...
		if(COM_BOOKINGFORCONNECTOR_GAENABLED == 1 && !empty(COM_BOOKINGFORCONNECTOR_GAACCOUNT) && COM_BOOKINGFORCONNECTOR_EECENABLED == 1) {
			...
			<script type="text/javascript">
			<!--
						if(typeof bookingfor_eeccreated == "undefined" || !bookingfor_eeccreated) {
							ga("require", "ec");
