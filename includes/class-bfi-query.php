...
//		}
	...
	public function init_query_vars() {
		...
		$this->query_vars = array(
			// Checkout actions.
			'eventdetails'          => get_option( 'eventdetails', 'eventdetails' ),
			'pointsofinterestdetails'          => get_option( 'pointsofinterestdetails', 'pointsofinterestdetails' ),				
			'merchantdetails'          => get_option( 'merchantdetails', 'merchantdetails' ),
			'resourcegroupdetails'          => get_option( 'resourcegroupdetails', 'resourcegroupdetails' ),
			'_api_controller'          => get_option( 'api_controller', 'api_controller' ),
			'resourcedetails'          => get_option( 'resourcedetails', 'resourcedetails' ),
			'experiencedetails'          => get_option( 'experiencedetails', 'experiencedetails' ),
			...
			'genericrequest'          => get_option( 'genericrequest', 'genericrequest' ),
		);
	}
	...
	public function get_endpoint_title( $endpoint ) {
		...
		switch ( $endpoint ) {
			case 'eventdetails' :
				$title = __( 'Event details', 'bfi' );
			break;
			case 'pointsofinterestdetails' :
				$title = __( 'PointsOfInterest details', 'bfi' );
			break;
			case 'merchantdetails' :
				$title = __( 'Merchant details', 'bfi' );
			break;
			case 'resourcegroupdetails' :
				$title = __( 'Resourcegroup Details', 'bfi' );
			break;
			case 'resourcedetails' :
				$title = __( 'Accomodation details', 'bfi' );
			break;
			case 'experiencedetails' :
				$title = __( 'Experience details', 'bfi' );
			break;
			case 'payment' :
	...
	public function add_endpoints() {
		foreach ( $this->query_vars as $key => $var ) {
			...
			if ( 'resourcegroupdetails' == $key ) {
				...
				$page_id   = bfi_get_page_id( 'resourcegroupdetails' ); 
				$page_slug = get_post( $page_id )->post_name;
				// e.g. /page-slug/my-endpoint/123/some-string/
			...
			if ( 'experiencedetails' == $key ) { //Experience
				...
				$page_id   = bfi_get_page_id( 'experiencedetails' ); 
				$page_slug = get_post( $page_id )->post_name;
				// e.g. /page-slug/my-endpoint/123/some-string/
			...
			if ( 'cartdetails' == $key ) {
				...
				add_rewrite_rule(
					"^(?:[a-z]{2}/|){$page_slug}/([^/-]*)(?:/)([^/]*)?$",
					'index.php?page_id=' . $page_id . '&bfi_layout=$matches[1]&orderid=$matches[2]',
					'top'
				);

			}
			if ( 'genericrequest' == $key ) {

				add_rewrite_tag( '%orderid%',   '([^&]+)' );

				$page_id   = bfi_get_page_id( 'genericrequest' ); 
				$page_slug = get_post( $page_id )->post_name;
			...
			if ( 'eventdetails' == $key ) {
				...
				$page_id   = bfi_get_page_id( 'eventdetails' ); 
				$page_slug = get_post( $page_id )->post_name;
				// e.g. /page-slug/my-endpoint/123/some-string/
			...
			if ( 'pointsofinterestdetails' == $key ) {
				...
				$page_id   = bfi_get_page_id( 'pointsofinterestdetails' ); 
				$page_slug = get_post( $page_id )->post_name;
				// e.g. /page-slug/my-endpoint/123/some-string/
