...
function bfi_disable_widgets( $sidebars_widgets ) { // remove sidebar

	if (BFI()->isEventPage() 
		|| BFI()->isCartPage() 
		|| BFI()->isSearchOnSellPage() 
		|| BFI()->isSearchEventsPage() 
		|| BFI()->isSearchPage() 
		|| BFI()->isResourcePage() 
		|| BFI()->isResourceOnSellPage() 
		|| BFI()->isResourcegroupPage() 
		|| BFI()->isMerchantPage()
	) {
			foreach ( $sidebars_widgets as $sidebar => $widgets ) {
				if ($sidebar !='bfisidebar' ) {
