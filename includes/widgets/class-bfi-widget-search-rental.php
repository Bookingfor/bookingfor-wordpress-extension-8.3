...
if ( ! class_exists( 'BFI_Widget_Booking_Search_Rental' ) ) {

	class BFI_Widget_Booking_Search_Rental extends WP_Widget {

		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->widget_cssclass    = 'bfi-widget_booking_search_rental';
			$this->widget_description = __( 'A Search box for Rental cars, bikes.', 'bfi' );
			$this->widget_id          = 'bookingfor_booking_search_rental';
			$this->widget_name        = __( 'BookingFor Search Rental', 'bfi' );
			$this->widget_sidebar    = 'bfisidebar Rental';
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'std'   => '',
	...
	<!-- 		<br />
	 ...
	 -->		<br />
			...
			<label class="checkbox"><input type="checkbox" name="<?php echo $this->get_field_name('showOnline'); ?>" value="1" <?php  echo ($showOnline=='1') ? 'checked="checked"' : ''; ?> /><?php _e('Only Online Booking', 'bfi'); ?></label>
			<br /><br />
		</p>
		<?php }  ?>
	...
	<?php if($resultinsamepg==1) { ?>[bookingfor_search_result_rental]
	<?php } ?>
	...
	//		  $instance['showSenior'] = ! empty( $new_instance[ 'showSenior' ] ) ? 1 : 0;
			  ...
			  if ($this->number=="__i__"){
				//echo "<p><strong>Widget ID is</strong>: Please save the widget</p>"   ;
						...
						$instanceContext = 'BookingFor Search Rental Widget - ' . $this->number;
		...
		public function widget( $args, $instance ) {
			...
			//chech widget to show
				...
				if (empty($widgettoshowFromsearch) || $widgettoshowFromsearch == $args["widget_id"]) {
					bfi_get_template("widgets/booking-search-rental.php",$args);
				}
