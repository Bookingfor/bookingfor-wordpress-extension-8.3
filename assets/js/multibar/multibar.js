    ...
    var createMarker = function(arValue,options){
        ...
        for(var i=0;i<arValue.length;i++){
            ...
            if(objValue){
                ...
                label = value;
				if(arValue[i].textalign && arValue[i].textalign==="left"){
					textalign = "text-align:left";
				}
				if(arValue[i].textalign && arValue[i].textalign==="right"){
					textalign = "text-align:right";
				}
				if (typeof arValue[i].label=='string')
				{
					label=arValue[i].label;
				}
            }
            else{
                value=arValue[i];
            ...
            marker+= '<div class="multi-bar-marker" style="' + direction + ':' + markerPosition + '%;color:' + color + '">' + 
                '<div class="multi-bar-marker-value" style="'+textalign+'">' + label + '</div>' + 
